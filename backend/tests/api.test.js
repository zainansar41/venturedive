...
describe("API routes", () => {
    ...
    it("responds to health endpoint", async () => {
        ...
        }, { summarise: async () => "summary", chat: async () => "chat" }, new ChunkerService({ chunkSize: 50, overlap: 10 }), { dimensions: 3, embed: async () => [[0.1, 0.2, 0.3]] }, {
            ensureIndex: async () => undefined,
            insertChunks: async () => undefined,
            vectorSearch: async () => [],
            deleteBySession: async () => undefined
        }, sessions);
        const chatController = new ChatController(new RagService({ dimensions: 3, embed: async () => [[0.1, 0.2, 0.3]] }, { summarise: async () => "summary", chat: async () => "answer" }, {
            ensureIndex: async () => undefined,
            insertChunks: async () => undefined,
            vectorSearch: async () => [],
            deleteBySession: async () => undefined
        }, { topK: 5, minScore: 0.7 }), sessions);
    ...
    it("handles ingest and chat endpoints", async () => {
        ...
        }, { summarise: async () => "Node summary", chat: async () => "Node summary" }, new ChunkerService({ chunkSize: 100, overlap: 10 }), { dimensions: 3, embed: async () => [[0.1, 0.2, 0.3]] }, {
            ensureIndex: async () => undefined,
            insertChunks: async () => undefined,
            vectorSearch: async () => [
                {
        ...
        const chatController = new ChatController(new RagService({ dimensions: 3, embed: async () => [[0.1, 0.2, 0.3]] }, { summarise: async () => "summary", chat: async () => "Ryan Dahl created Node.js." }, {
            ensureIndex: async () => undefined,
            insertChunks: async () => undefined,
            vectorSearch: async () => [
                {
